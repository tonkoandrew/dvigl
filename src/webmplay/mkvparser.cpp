...
template <typename Type> Type* SafeArrayAlloc(unsigned long long num_elements, unsigned long long element_size)
{
...
bool Cues::Find(long long time_ns, const Track* pTrack, const CuePoint*& pCP, const CuePoint::TrackPosition*& pTP) const
{
...
long Segment::DoParseNext(const Cluster*& pResult, long long& pos, long& len)
{
    ...
    if (status > 0)
    { // means "found at least one block entry"
...
Chapters::Chapters(
    Segment* pSegment, long long payload_start, long long payload_size, long long element_start, long long element_size)
    : m_pSegment(pSegment)
...
SegmentInfo::SegmentInfo(
    Segment* pSegment, long long start, long long size_, long long element_start, long long element_size)
    : m_pSegment(pSegment)
    , m_start(start)
    , m_size(size_)
    , m_element_start(element_start)
...
long ContentEncoding::ParseContentEncAESSettingsEntry(
    long long start, long long size, IMkvReader* pReader, ContentEncAESSettings* aes)
{
    assert(pReader);
    assert(aes);
...
long ContentEncoding::ParseCompressionEntry(
    long long start, long long size, IMkvReader* pReader, ContentCompression* compression)
{
    assert(pReader);
    assert(compression);
...
long ContentEncoding::ParseEncryptionEntry(
    long long start, long long size, IMkvReader* pReader, ContentEncryption* encryption)
{
    assert(pReader);
    assert(encryption);
...
long Track::Create(
    Segment* pSegment, const Info& info, long long element_start, long long element_size, Track*& pResult)
{
    if (pResult)
        return -1;
...
bool PrimaryChromaticity::Parse(
    IMkvReader* reader, long long read_pos, long long value_size, bool is_x, PrimaryChromaticity** chromaticity)
{
    if (!reader)
        return false;
    ...
    const long long value_parse_status = UnserializeFloat(reader, read_pos, value_size, parser_value);
...
bool MasteringMetadata::Parse(IMkvReader* reader, long long mm_start, long long mm_size, MasteringMetadata** mm)
{
    ...
    while (read_pos < mm_end)
    {
        ...
        else
        {
            ...
            const bool value_parse_status
                = PrimaryChromaticity::Parse(reader, read_pos, child_size, is_x, chromaticity);
...
bool Colour::Parse(IMkvReader* reader, long long colour_start, long long colour_size, Colour** colour)
{
    ...
    while (read_pos < colour_end)
    {
        ...
        {
            if (!MasteringMetadata::Parse(reader, read_pos, child_size, &colour_ptr->mastering_metadata))
                return false;
        }
        else
        {
...
long VideoTrack::Parse(
    Segment* pSegment, const Info& info, long long element_start, long long element_size, VideoTrack*& pResult)
{
    if (pResult)
        return -1;
...
long AudioTrack::Parse(
    Segment* pSegment, const Info& info, long long element_start, long long element_size, AudioTrack*& pResult)
{
    if (pResult)
        return -1;
...
long Tracks::ParseTrackEntry(
    long long track_start, long long track_size, long long element_start, long long element_size, Track*& pResult) const
{
    if (pResult)
        return -1;
...
long Cluster::ParseSimpleBlock(long long block_size, long long& pos, long& len)
{
    ...
    status = CreateBlock(libwebm::kMkvSimpleBlock, block_start, block_size, 0); // DiscardPadding
