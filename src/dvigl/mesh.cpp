    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (index >= (int)mesh->mNumVertices) {
        ...
        if (mesh->mTextureCoords[0] != NULL) {
          ...
          textureCoord_data[tcoord_index + 1] =
...
void Mesh::genVAO() {
  if (vertex_count > 0) {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertex_count * 4, vertex_data,
                 GL_STATIC_DRAW);
    ...
    glGenBuffers(1, &buffer);
    glBindBuffer(GL_ARRAY_BUFFER, buffer);
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertex_count * 3, normal_data,
                 GL_STATIC_DRAW);
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertex_count * 2,
                 textureCoord_data, GL_STATIC_DRAW);
