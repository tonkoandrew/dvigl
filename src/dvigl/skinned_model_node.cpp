...
void ozz::io::Extern<dvigl::Mesh::Part>::Load(IArchive& _archive,
                                      dvigl::Mesh::Part* _parts, size_t _count,
                                      uint32_t _version)
{
  (void)_version;
  for (size_t i = 0; i < _count; ++i) {
...
// ==================== AAA
  ...
  for (size_t i = 0; i < _mesh.parts.size(); ++i) {
    ...
    for (size_t j = 0; j < part_vertex_count; j += OZZ_ARRAY_SIZE(kDefaultColorsArray))
    {
        ...
        memcpy(
            ozz::PointerStride(vbo_map, colors_offset + (processed_vertex_count + j) * colors_stride),
            kDefaultColorsArray,
      ...
      if (part_vertex_count == part.uvs.size() / dvigl::Mesh::Part::kUVsCpnts)
      {
        ...
        memcpy(
            ozz::PointerStride(vbo_map, uvs_offset + processed_vertex_count * uvs_stride),
        ...
        for (size_t j = 0; j < part_vertex_count; j += OZZ_ARRAY_SIZE(kDefaultUVsArray))
        {
          ...
          memcpy(
            ozz::PointerStride(vbo_map, uvs_offset + (processed_vertex_count + j) * uvs_stride),
            kDefaultUVsArray,
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                indices.size() * sizeof(dvigl::Mesh::TriangleIndices::value_type),
