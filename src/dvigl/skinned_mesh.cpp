...
SkinnedMesh::SkinnedMesh(const aiScene* scene)
{
    ...
    TicksPerSecond = (float)(m_pScene->mAnimations[0]->mTicksPerSecond != 0
...
bool SkinnedMesh::InitFromScene(const aiScene* scene)
{
    ...
    glBufferData(GL_ARRAY_BUFFER,
        sizeof(Bones[0]) * Bones.size(),
        &Bones[0],
        GL_STATIC_DRAW);
    glEnableVertexAttribArray(BONE_ID_LOCATION);
    glVertexAttribIPointer(
        BONE_ID_LOCATION, 4, GL_INT, sizeof(VertexBoneData), (const GLvoid*)0);
...
void SkinnedMesh::LoadBones(GLuint MeshIndex,
    const aiMesh* pMesh,
    vector<VertexBoneData>& Bones)
{
...
GLuint
SkinnedMesh::FindPosition(float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
GLuint
SkinnedMesh::FindRotation(float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
GLuint
SkinnedMesh::FindScaling(float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
void SkinnedMesh::CalcInterpolatedPosition(aiVector3D& Out,
    float AnimationTime,
...
void SkinnedMesh::CalcInterpolatedScaling(aiVector3D& Out,
    float AnimationTime,
    const aiNodeAnim* pNodeAnim)
{
